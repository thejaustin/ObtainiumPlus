...
Traced every place `app.id` gets its value (not just the sinks) rather than patching each of the ~10 file-path call sites individually — cheaper and covers dead code too. Found 4 real ingestion points, all now sanitized:
- `SourceProvider._resolveAppId()` (`lib/providers/source_provider.dart`) — covers the explicit `additionalSettings['appId']` override and `source.tryInferringAppId()` (the GitHub source parses this from the repo's `build.gradle` `applicationId` — fully attacker-controlled).
- `App.fromJson()` (`lib/models/app.dart`) — covers import (a shared/malicious export file).
- `FDroidRepo.postProcessApp()` (`lib/app_sources/fdroidrepo.dart:144`) — sets `id` from a URL query parameter via `copyWith`, bypassing `_resolveAppId` entirely; missed by the 07-21 review.
- `add_app.dart:355` — sets `id` from a downloaded artifact's `appId` post-download, also bypassing `_resolveAppId`; also missed on 07-21.

Initially left `apps_provider.dart:1021`/`app_download_service.dart:71` (`app.id = newInfo.packageName!`) unsanitized, reasoning it was Android's own installed-package name — **wrong, corrected same session**: a multi-angle code-review pass (`/code-review medium`, 8 finder angles + verify) traced `newInfo` back to `packageManager.getPackageArchiveInfo(archiveFilePath: ...)`, which parses the *downloaded-but-not-yet-installed* APK's manifest directly, not a verified installed-package query. That's attacker-influenced the same way the other four ingestion points are (a crafted manifest `package` value accepted by the lenient archive parser but rejected by the real installer). Fixed both live call sites (`apps_provider.dart:1021`, `app_download_service.dart:71`) — 6 ingestion points sanitized in total now.

Also confirmed (via a dedicated Explore agent, same "CI green ≠ every file compiles" class of issue as 07-12) that most of the split-out service files are dead code: `apps_provider_lifecycle.dart` and `apps_provider_install.dart` are `extension ... on AppsProvider` blocks whose method names collide with the real `AppsProvider` class methods, so Dart's external-call resolution always picks the class method — the extensions are unreachable except from within themselves. `app_crud_service.dart` is imported by nothing at all. Didn't touch these (no functional impact, out of scope for this fix) but worth a follow-up cleanup pass sometime — they're a standing trap for future "fix the file that looks canonical" mistakes. `app_download_service.dart` is the one split-out file that's actually live (delegated to from `apps_provider.dart` and called directly by `background_update_service.dart`).

**Code review pass (medium effort, 8 angles + 1-vote verify) on the whole diff, before commit.** Two more real fixes came out of it beyond the archive-parse gap above:
- `source_provider.dart`'s `_resolveAppId` originally wrapped `sanitizeAppId(...)` around 3 of its 4 return statements individually — refactored to resolve one raw value and sanitize once at a single return, so a future 5th branch can't forget the wrapper. (First attempt at this refactor accidentally called the async `source.tryInferringAppId(...)` twice — once in a condition, once to grab the value — caught before committing; final version calls it once.)
- `sanitizeAppId` itself collapsed from 4 chained `.replaceAll` calls to 2 regex passes, no behavior change.
- `.github/workflows/sentry-sync.yml`'s new 404-handling branch (see below) was flagged independently by 3 of the 8 review angles as a mass-false-close risk — reworked to a two-strike pattern (see below) instead of closing on the first 404.

Not fixed, judged out of scope or low-severity with documented rationale (see the code-review report for this session): `sanitizeAppId`'s `/`→`_` and `\`→`_` collapse is many-to-one, so two *already-malicious* raw ids could collide onto the same sanitized id and overwrite each other's local files — contained blast radius (only reachable when a raw id is already unsafe), not fixed. A pre-fix app already saved to disk under an unsafe id would load under a new sanitized id without its old file being renamed — only reachable if already exploited before this fix shipped, not fixed. `App.id` remains a plain mutable public field rather than an encapsulated/validated property — real architectural point (a future 7th ingestion point has no compiler signal to sanitize), but a bigger refactor than this patch's scope; noted for a future session.

Also fixed (found while checking the one open GH issue, #179, a `DownloadCancelledError` sentry-crash issue stuck reopening in a loop even though the root cause was fixed 07-21): `sentry-sync.yml`'s close-detection loop only ever closed a GH issue when Sentry explicitly returned `status: resolved`; a 404 (Sentry issue deleted/merged/pruned, as happened here — confirmed directly against the Sentry API/MCP that OBTAINIUMPLUS-1H no longer exists) just logged a warning and skipped forever, so it could never self-heal. Closed #179 by hand and fixed the workflow, but the first pass (auto-close immediately on 404) was itself flagged by 3 independent review angles as unsafe: Sentry returns 404 for auth/scope failures too (an expired/rescoped `SENTRY_AUTH_TOKEN` or drifted `SENTRY_ORG`/`SENTRY_PROJECT` secret would 404 on *every* open issue in the same run), which would have mass-closed the whole crash-tracking backlog with a false "resolved" comment on a single bad run. Reworked to a two-strike pattern instead: first 404 posts a warning comment with a hidden `<!-- sentry-404-strike -->` marker and does *not* close; only closes on a second 404 (i.e. the marker already present from a prior run), so a transient misconfiguration only ever warns.

Verified with `scripts/dev/check-syntax.sh` (parser-only, no local Flutter SDK) and a manual JS-syntax extraction check (`node --check`) on the sentry-sync.yml embedded script, since that file isn't Dart and the syntax checker doesn't cover it. Uncommitted per session convention — commits are user-triggered. Still no device/wifi access (online via mobile data this continuation, wifi/ADB still unavailable).
...
**Bottom nav made default UI + overlap/mutual-exclusivity fixes; live device testing blocked by wifi, so also researched Samsung ADB blockers for next time.**

- **Bottom nav + FAB default-on**: `plusEnableBottomNavBar` now defaults `true` in `plus_settings_provider.dart` (`plusEnableFAB` was already `true`). New installs get bottom nav + FAB out of the box.
- **One settings entry point at a time**: `apps.dart` had its own unconditional settings gear icon in *both* AppBar variants (`SliverAppBar.large` and the compact `CustomAppBar`), on top of whatever `home.dart` already shows for the active nav mode (top TabBar gear, or bottom-nav Settings tab). Added `hasExternalSettingsEntry = (plusTopUILayout && !isTV) || plusEnableBottomNavBar` and gated both gear icons on `!hasExternalSettingsEntry`.
- **Bottom nav no longer overlaps content**: `home.dart`'s outer `Scaffold` uses `extendBody: true` so the translucent/blurred bottom nav bar shows page content through it (frosted-glass look) — but nothing padded that content by the bar's height, so the last list items and the Apps page's `persistentFooterButtons` row rendered *underneath* the bar instead of above it. Fixed by keying the bar's `ClipRRect`, measuring its real rendered height via `addPostFrameCallback`, and padding `pageBody` by that amount whenever `showNavBar` is true. Avoided hardcoding a height constant since it depends on theme/font-scale/safe-area.
- **Samsung ADB/USB-debugging blockers researched** (device testing was blocked by "no wifi" this session — for next time): Samsung's **Auto Blocker** (One UI 6+, Android 14+, shipped 2023-10-31) can grey out Developer Options / block ADB commands over USB entirely when enabled. Fix: **Settings → Security and Privacy → Auto Blocker → off** (temporarily), then Developer Options should re-enable. Also: when the USB-debugging authorization dialog appears, check "Always allow from this computer" to avoid re-prompting every reconnect. Note this session's actual overlay problem (floating Termux "chat head" notification bubbles from *other* concurrent Claude/Gemini sessions on the same physical device stealing taps/focus) was **not** a Samsung/Knox restriction — it's an artifact of running Claude Code inside Termux/PRoot on the same device being tested. Workaround used: `adb shell input keyevent KEYCODE_BACK`/`KEYCODE_HOME` to dismiss the overlay, then `adb shell am start -n <pkg>/.MainActivity` (explicit component) to force foreground, verified via `dumpsys activity activities | grep topResumedActivity` before any further taps.
- Pushed as `cb19c467` (rebased cleanly onto CI auto-bump `00261b71`); Build APK, Lint Commit Messages, and Fastlane validation all green.
- **Still open, blocked on device/wifi access returning**: Shizuku toggle in Settings reported not working (static read looks structurally sound — needs live tap-through to see actual failure mode); FAB Menu / App Tile Display / Search Settings sections crash with "type 'bool' is not a subtype of type 'double?' in type cast" — root cause still unknown after exhaustive static analysis and a 130k-line logcat search turned up nothing, and Sentry isn't capturing it (likely a build-phase `ErrorWidget` error, not an uncaught exception it auto-instruments); `main.dart`'s `ErrorWidget.builder` was instrumented to log `details.exception`/`details.stack` via `LogsProvider` (commit `675985a5`, already shipped) — next session should reproduce the crash on-device and read the real stack trace via Settings → Troubleshooting & System → App Logs.
